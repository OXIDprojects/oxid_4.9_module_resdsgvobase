...
[{block name="oegdprbase_add_css"}]
    ...
    [{if $oViewConf->getActiveTheme() == 'azure'}]
        [{oxstyle include=$oViewConf->getModuleUrl('resGDPRBase','out/css/oegdprbase-azure.css')}]
    [{else}]
        [{oxstyle include=$oViewConf->getModuleUrl('resGDPRBase','out/css/oegdprbase-flow.css')}]
    [{/if}]
[{/block}]
